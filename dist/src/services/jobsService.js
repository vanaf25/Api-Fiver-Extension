...
export class JobsService {
    static async createJob(body, userId) {
        const response = await scrapper.getGigData(body.url);
    ...
    static async applyForJob(jobId, userId, isExchange) {
        ...
        job.currentJob.forEach((job) => {
            if (job === userId)
                return ApiError.UnauthorizedError("Recently You applied for this job");
        });
        if (!job) {
    ...
    static async updateCurrentJob(jobId, userId, steps) {
        ...
        const currentJob = await CurrentJobModel.findOne({ _id: jobId }).populate('job')
            .exec();
        ...
        if (currentJob.isComplete)
            return ApiError.NotFound("This Job already completed ");
        ...
        if (isCompleted) {
            ...
            await JobModel.findOneAndUpdate({ _id: currentJob.job._id }, countOfCompleted);
        }
        await CurrentJobModel.findOneAndUpdate({ _id: jobId }, { ...requiredThings, isComplete: isCompleted }, { new: true });
    ...
    static async removeCurrentJob(jobId) {
        ...
        if (!currentJob)
            return ApiError.NotFound("The currentJob weas not founded");
        await CurrentJobModel.deleteOne({ _id: jobId });
    }
    static async exchangeJobs(jobId, userId) {
        const job = await JobModel.findOne({ _id: jobId });
        if (!job)
            throw ApiError.NotFound("The job was not founded");
    ...
    static async getCurrentJob(currentJobId) {
        ...
        if (!currentJob)
            return ApiError.NotFound("The current Job was not founded");
        return currentJob;
    }
    static async applyForExchange(exchangeId, jobId, userId) {
        ...
        if (!job)
            return ApiError.NotFound("The job was not founded");
        if (!exchange)
            throw ApiError.NotFound("The exchange was not found");
        if (exchange.secondJob)
            return ApiError.defaultError("You can't apply for this exchange");
        const currentJob = await this.applyForJob(jobId, userId, true);
        ...
        await ExchangeModel.findOneAndUpdate({ _id: exchangeId }, { secondJob: jobId }, { new: true });
    ...
    static async getExchanges(userId) {
        let exchanges = await ExchangeModel.find({ secondUser: userId })
        ...
        let myExchanges = await ExchangeModel.find({ user: userId })
            .populate({ path: "firstJob", populate: {
                path: 'currentJob',
                model: "currentJob",
            } })
